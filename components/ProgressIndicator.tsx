...
const steps: { number: 1 | 2 | 3; label: string; icon: string }[] = [
...
export function ProgressIndicator({ currentStep }: { currentStep: number }) {
  const { firstStep, secondStep, setCurrentStep } = useBookingStore();
  ...
  const isStepReachable = (step: number) => step < currentStep;
    ...
    <div className="mb-8">
      <div className="flex items-center justify-between">
        {steps.map((step, idx) => {
          ...
          const reachable = isStepReachable(step.number);
              ...
              <div className="relative z-10 flex flex-col items-center">
                <button
                  ...
                  className={`
                    ...
                    ${reachable ? 'cursor-pointer hover:ring-4 hover:ring-blue-100' : 'cursor-default'}
